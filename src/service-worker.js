...
// Checks the connection state to the network
...
self.addEventListener('install', function (event) {
	event.waitUntil(
		console.log("Installation of service worker.");
	...
	// Check the state of connection to the network
	checkNetworkState();
});
...
self.addEventListener('fetch', function (event) {
	...
	if(event.request.method === "POST"){
		console.log("We just captured a post!")
		console.log(event.request);
	}
});

// Index DB handling for offline posts
async function sendOfflinePostRequestsToServer() {
	var request = indexedDB.open("ui5TodoDB");
